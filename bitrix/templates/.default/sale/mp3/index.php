...
global $mp3FolderName, $mp3Price, $mp3Currency, $mp3AuxiliaryPrefix, $mp3AccessTimeLength, $mp3AccessTimeType, $bMP3ConvertCurrency, $arMP3Sums, $mp3Path2Folder, $mp3Url2Folder, $mp3Path2Original, $mp3Url2Original, $arMP3Genres, $arMP3Version, $arMP3Layer, $arMP3CRC, $arMP3Bitrate, $arMP3BitIndex, $arMP3Freq, $arMP3Mode, $arMP3Copy;

$APPLICATION->IncludeFile("lang/".LANG."/sale/mp3/mp3.php");
...
if ($USER->IsAuthorized() && ($action == "buy"))
{
	...
	if (strlen($errorMessage) <= 0)
	{
		...
		for ($i = 0; $i < count($buy_mp3); $i++)
		{
			...
			while (substr($buy_mp3[$i], 0, 1) == ".")
				$buy_mp3[$i] = substr($buy_mp3[$i], 1);
			if (strlen($buy_mp3[$i]) > 0)
...
if (is_dir($mp3Path2Original))
{
	...
	{
		if (filetype($mp3Path2Original.$fileName) == "file" && substr($fileName, 0, 1) != ".")
		{
