...
<!-- сортировка -->
<!--
<div style="padding: 2px 2px 2px 16px; margin:0 0 25px 0; width:180px; float:right; background-color:#FDF7EA; border-radius:10px; border-color: #f79b1c; border-width: 2px; border-style: solid; box-shadow: 1px 1px 5px 1px #aaaaaa;">
-->
<div style="float:right; padding: 10px 2px 2px 16px; margin:0 0 15px 0; background-color:#FDF7EA; border-radius:10px; border-color: #f79b1c; border-width: 1px; border-style: solid;">
<div style="margin:0 0 5px 0; text-align:center">Сортировка: </div>
<table>
<tr>
	<td> ID </td>
	<td> Статус</td>
...
-->							
							...
							<?if ($arParams["SHOW_STATUS"]!="Y") { ?><br /><?=SortingEx("s_id")?><? } //endif($SHOW_STATUS!="Y");?></th>
							<?
							if ($arParams["SHOW_STATUS"]=="Y") 
							{
							?>
						...
						?>
						<tr>
							<th><br /><?=GetMessage("FORM_STATUS")?></th>
							<td><?=SortingEx("s_status")?></td>
						</tr>
						<? 
						} //endif($SHOW_STATUS=="Y");
				...
				<?
/* столбец с юзером				if ($arParams["F_RIGHT"] >= 25) 
				{ 
				?>
				<th>
					<table class="form-results-header-inline">
					...
					foreach ($arResult["arrColumns"] as $arrCol)
					{
						if (!is_array($arParams["arrNOT_SHOW_TABLE"]) || !in_array($arrCol["SID"], $arParams["arrNOT_SHOW_TABLE"])) 
						{
							if (($arrCol["ADDITIONAL"]=="Y" && $arParams["SHOW_ADDITIONAL"]=="Y") || $arrCol["ADDITIONAL"]!="Y") 
							{
								...
								?>
				<th>
<!-- ненужная ссылка на редактирование поля 				
								<?
								if ($arParams["F_RIGHT"] >= 25) 
								{
								?>
		...
		************************************************/
		...
		if(count($arResult["arrResults"]) > 0)
		{
			...
			?>
				<tr>
					<td colspan="<?=$colspan?>"><?
/*	чекбокс				if ($arParams["can_delete_some"] && $arRes["can_delete"]) 
					{
						?><input type="checkbox" name="ARR_RESULT[]" value="<?=$arRes["ID"]?>" /><?
					} //endif ($can_delete_some && $can_delete);   */
					?><input type="hidden" name="RESULT_ID[]" value="<?=$arRes["ID"]?>" />ID:&nbsp;<b><?=($arParams["USER_ID"]==$arRes["USER_ID"]) ? "<span class='form-result-id'>".$arRes["ID"]."</span>" : $arRes["ID"]?></b> &nbsp;
<!-- выводим статус -->
					<? 
					if ($arParams["SHOW_STATUS"] == "Y") 
					{
			...
			?>
				<tr>
					<td>
						...
						?>
						<?
/* ссылка Удалить						if ($arRes["can_delete"]) 
						{
