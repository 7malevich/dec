...
function showMenuR($items){
	foreach($items as $i=>$item){
		...
		if($item['DEPTH_LEVEL'] <= $GLOBALS['stopLevel']){
			if ( $item['DEPTH_LEVEL']==2 && $item['SELECTED'] && $item['items'] ){
			printf(
				'<li class="typical-aside-nav__item" style="margin-bottom:40px"><a class="%s" href="%s"><span>В этом разделе:</span></a>',
				$addClass,
				$item['LINK'],
			);
			}
			if($item['DEPTH_LEVEL']>2){ //не выводим первые 2 уровня
			printf(
...
?>
<?if (!empty($arResult)){?>
<ul class="sideMenu level-1 mb-0">
<?
$previousLevel = 0;
$itemDisplayed = false;
