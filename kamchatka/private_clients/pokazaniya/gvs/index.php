...
$APPLICATION->SetTitle("Передача показаний онлайн (горячее&nbsp;водоснабжение)");
?>
<?php
...
function Teplosave_readings($connect, $sessionkey, $reading, $mpi) {
  ...
  $stmt = null;
  $connect = null; //разрываем соединение с БД
  $nextpage='Location: received.php?mpi='.intval(date_count($mpi));
  header($nextpage);
}
...
<img src="/images/design2016/icons/kran_voda.svg" style="display:block;float: left;margin: 4px 0 0 0;width: 48px;height: 48px">
					...
					<?php include('schedule.php'); ?>
                        ...
                        <?php elseif ((strlen($_POST['reading1']) != $meter['capacity']) ): ?>
                            ...
                            <?php elseif ($_POST['step'] == 3): ?>
							  ...
							  <?php Teplosave_readings($connect, $_POST['sessionkey'], number_format(floatval(substr_replace($_POST['reading1'], '.', $_POST['capacity1'], 0)), 4, '.', ''), $_POST['mpi']) ?>
