...
function func1(xhttp) {
	...
	if(typeof(xhttp.data) != 'undefined') {
		for (i = 0; i < xhttp.data.length; i++) {
			...
			loadDoc('queue/sendwarning.php?n=timeout', func_blank);
			console.log( 'ошибка: нет данных от сервера');
		}
	if (xhttp.msg_err) {
		console.log( 'ошибка: '.xhttp.msg_err);
	}; 
	if (slots == 0) { //если в списке услуг вооще нет свободных слотов - вывод сообщения
		...
		document.getElementById("form1").style.display = 'none';
			loadDoc('queue/sendwarning.php?n=noslots', func_blank);
			console.log( 'ошибка: нет слотов');
...
function func4(xhttp) {
	...
	if (xhttp.status == "ok") {
		...
		loadDoc('queue/sendwarning.php?n=OK', func_blank);
