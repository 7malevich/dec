...
<?php if ($_COOKIE['dvec_meter_mpi_teplo']): ?>
<?php if ((intval($_COOKIE['dvec_meter_mpi_teplo']) > 0) && (intval($_COOKIE['dvec_meter_mpi_teplo']) < 91)): ?>
	<p style="text-align:center">Уважаемый клиент!</p>
	<p>У Вашего прибора учета подходит к концу межповерочный интервал <span style="background-color:#f8bbb4">(дней осталось: <?php echo($_COOKIE['dvec_meter_mpi_teplo'])?>)</span>, и в скором времени Вам будут приходить счета по среднему потреблению, а затем по нормативам.</p>
...
<?php if (intval($_COOKIE['dvec_meter_mpi_teplo']) <= 0): ?>
	<p style="text-align:center">Уважаемый клиент!</p>
