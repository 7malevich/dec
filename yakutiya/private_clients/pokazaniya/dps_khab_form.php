...
<?php if ((intval($_COOKIE['dvec_meter_mpi']) > 0) && (intval($_COOKIE['dvec_meter_mpi']) < 91)): ?>
	...
	<p>У Вашего прибора учета подходит к концу межповерочный интервал <span style="background-color:#f8bbb4">(дней осталось: <?php echo($_COOKIE['dvec_meter_mpi'])?>)</span>, и в скором времени Вам будут приходить счета по среднему потреблению, а затем по нормативам.</p>
